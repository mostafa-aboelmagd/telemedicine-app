...
const menuIcon = <div><IoMenu className="h-8 w-8 text-[#035fe9]" /></div>;
const signedInIcon = <div><FaUserCircle className="h-10 w-10 text-[#035fe9]" /></div>
const Navbar = () => {
      ...
      <div className="max-w-full md:max-w-[90%] lg:max-w-[75%] flex justify-between items-center mx-auto">
        ...
        <div className="flex justify-between items-center space-x-0 md:space-x-4 lg:space-x-6">
          ...
          {!token! ? <><Link href="/auth/signin">
            ...
            :
            <div>
              <MenuList linkTo={["/patientProfile/view", "/patientProfile/prescriptions", "/patientProfile/patientDocuments", "/patientProfile/paymentInfo"]} linkName={["View Profile", "My Prescriptions", "My Documents", "Wallet"]} text={signedInIcon} />
            </div>
          }
