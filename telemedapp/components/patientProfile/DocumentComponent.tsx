...
const DocumentComponent = ({ document, icon }: { document: any, icon: any }) => {
    ...
    return (
        <div className='bg-gray-100 rounded-3xl p-4 flex justify-between items-center'>
            <div onClick={handleOpenDocument} className='flex space-x-4 items-center cursor-pointer'>
            ...
            <div onClick={handleDownloadDocument}>
                <MdDownload className='cursor-pointer bg-transparent h-10 w-10 rounded-full p-2 text-[#035fe9]' />
            </div>
        </div>
    )
}
