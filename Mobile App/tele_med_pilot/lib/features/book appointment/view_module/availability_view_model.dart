...
class AvailabilityViewModel {
  ...
  Future<List<AvailableViewModel>> fetchDoctorAvailability(int doctorId) async {
    Availabilities availabilities = await bookAppointmentRepo!.getAvailability(doctorId);
    ...
    availabilities.availabilities.forEach((date, availabilityList) {
      availableViewModels.addAll(
        availabilityList.map((availability) => AvailableViewModel(availabilityModel: availability)).toList(),
      );
    });
