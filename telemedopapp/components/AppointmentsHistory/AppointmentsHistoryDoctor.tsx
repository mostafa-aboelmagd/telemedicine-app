...
const AppointmentsHistory = ({ userId }: AppointmentsHistoryProps) => {
    ...
    const fetchAppointments = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_NAME}/backOffice/getDoctorAppointments/${userId}`,
          {
            method: 'GET',
            headers: {
        ...
        if (data.status === "success" && data.doctorAppointments) {
          setAppointmentsData(data.doctorAppointments);
        }
      } catch (error) {
    ...
    return (
      <div className="w-full max-w-6xl mx-auto p-4">
        ...
        <div className="bg-gray-50 rounded-lg p-8 text-center">
          <p className="text-gray-600 text-lg">No appointments found in doctor's history.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4">
      <h2 className="text-2xl font-bold mb-6">Appointments History - Dr {appointmentsData[0].doctor_first_name} {appointmentsData[0].doctor_last_name}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-gray-600">
                  <span className="font-medium">Birth Date:</span> {formatBirthDate(appointment.patient_birth_date)}
                </p>
                <p className="text-gray-600">
                  <span className="font-medium">Contact:</span> {appointment.patient_phone_number}
                </p>
                <p className="text-gray-600">
