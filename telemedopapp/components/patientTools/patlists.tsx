...
const Patlists = () => {
  ...
  const handleAddMessage = () => {
    if (customMessage.trim()) {
      ...
      setCustomMessage(""); // Clear the input after adding
    }
  };

  const handleSendReminder = async (patient: any, message: string) => {

    if (!isMessageSelected()) {
      return; // Exit early if no message is selected
    }
  
    if (isPatientProfileIncomplete(patient)) {
        await sendEmail({
            sendToEmail: [patient.user_email],
            subject: "Profile Completion Reminder",
            message: selectedMessage || customMessage,
        });
    } 
    // console.log(patient.user_email);
                ...
                <div className="min-[1350px]:grid-cols-1 p-3 gap-y-10 justify-items-center">
                  ...
                  {selectedPatient && (
                    <div className="fixed inset-0 flex items-center justify-center z-50">
                      <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-3xl overflow-y-auto max-h-[80vh] relative">
                        <button
                          className="absolute top-2 right-2 text-red-600 hover:text-red-800 text-2xl p-2"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/50">
          <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
            ...
            {/* Label for adding a custom message */}
            ...
            <div className="mt-4 flex justify-end">
              <button
                ...
                onClick={() => {
                  handleSendReminder(selectedPatient, selectedMessage || customMessage);
